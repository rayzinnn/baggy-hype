  ...
  return (
    <div className="flex-1 w-full bg-black flex flex-col overflow-x-hidden selection:bg-primary/40 selection:text-white">
      ...
      <section className="relative min-h-[85vh] flex flex-col items-center justify-center p-4">
        {/* Massive Background Text - "PALMAS" */}
        ...
        <div className="relative z-10 w-full max-w-7xl mx-auto flex flex-col md:flex-row items-center gap-12 mt-12">
            <div className="flex-1 flex flex-col gap-6 text-left order-2 md:order-1 px-4">
                <div className="flex items-center gap-2">
                    ...
                    <span className="text-white font-black uppercase text-[10px] md:text-xs tracking-[0.5em]">Palmas TO • Streetwear</span>
                </div>
                <h1 className="text-7xl md:text-[11rem] font-black italic tracking-tighter uppercase leading-[0.8] text-white">
                    DRIP <br/><span className="text-primary not-italic">BRUTAL</span>
                </h1>
                <p className="text-white/40 text-sm md:text-xl font-medium max-w-md leading-relaxed mt-4 lowercase italic">
                    Fragmentos da alma urbana em forma de tecido. <br className="hidden md:block" /> Conforto absurdo para o clima mais quente do cerrado.
                </p>
                <div className="flex flex-col sm:flex-row gap-4 mt-8">
                    <Link href="/catalog" className="px-10 py-5 bg-white text-black font-black uppercase text-[10px] tracking-widest hover:bg-primary transition-all flex items-center justify-center gap-2 group border border-white">
                        Explorar Catálogo
                        <ArrowRightIcon className="w-4 h-4 group-hover:translate-x-1 transition-transform" />
                    </Link>
                    <Link href="/drops" className="px-10 py-5 border border-white/20 text-white font-black uppercase text-[10px] tracking-widest hover:border-white transition-all flex items-center justify-center">
                        Coleção 2026
                    </Link>
                </div>
            </div>

            {/* Hero Image (DYNAMIC FROM CONFIG) */}
      ...
      {/* Brand Ethos */}
      <section className="py-32 border-y border-white/5 flex flex-col gap-12 overflow-hidden">
         <div className="flex animate-infinite-scroll whitespace-nowrap gap-8 py-4">
            {[1,2,3].map(i => (
                <div key={i} className="flex gap-8 items-center">
                    <span className="text-7xl md:text-9xl font-black italic uppercase tracking-tighter text-transparent border-text-white drop-shadow-[0_0_2px_rgba(255,255,255,0.2)]">#BaggyHypeClub</span>
                    <span className="text-7xl md:text-9xl font-black uppercase tracking-tighter text-primary">063_PALMAS</span>
                    <span className="text-7xl md:text-9xl font-black italic uppercase tracking-tighter text-white">OVERSIZED_ONLY</span>
                </div>
            ))}
         </div>
      </section>
      ...
      <section className="py-24 px-4 md:px-8 max-w-7xl mx-auto w-full" id="drops">
         <div className="flex items-end justify-between mb-20 border-l-4 border-primary pl-6">
            <div className="flex flex-col gap-2">
               <h2 className="text-6xl md:text-8xl font-black uppercase italic tracking-tighter leading-none text-white">ÚLTIMOS <span className="text-primary italic">DROPS</span></h2>
               <p className="text-white/30 text-[10px] font-bold uppercase tracking-[0.3em]">Exclusividade • Conforto • Atitude</p>
            </div>
            <Link href="/catalog" className="hidden md:block text-white text-[10px] font-black uppercase tracking-widest border-b border-primary pb-1 hover:text-primary transition-colors">
               Ver Tudo
            </Link>
         </div>
         ...
         <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-x-1 gap-y-16">
            {featuredProducts.length === 0 ? (
                <div className="col-span-full py-20 text-center flex flex-col items-center gap-4 opacity-20">
                    <p className="text-[10px] font-black uppercase tracking-[0.5em]">Nenhum drop disponível no momento.</p>
                </div>
            ) : (
                        ...
                        <Link href={`/product/${product.slug || product.id}`} key={product.id} className="group flex flex-col gap-4">
                            <div className="aspect-[3/4] bg-neutral-900 overflow-hidden relative border border-white/5">
                                ...
                                <div className="absolute top-0 right-0 p-3 flex flex-col items-end gap-1">
                                    <span className="bg-primary text-black font-black text-[8px] uppercase px-3 py-1 block shadow-lg">
                                        DROP OURO
                                    </span>
                                </div>
                            </div>
         ...
         <div className="relative z-10 max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between gap-8 text-black">
            ...
            <div className="flex flex-col items-center md:items-end gap-2 text-center md:text-right">
                <p className="text-[10px] font-black uppercase tracking-widest max-w-xs leading-relaxed">
                    Compre e receba hoje em qualquer setor da capital. O estilo não pode esperar.
                </p>
                <div className="bg-black text-white px-8 py-3 font-black uppercase text-[10px] tracking-widest hover:bg-white hover:text-black transition-all cursor-pointer border border-black group">
                    Pedir pelo WhatsApp <ArrowRightIcon className="inline-block w-3 h-3 group-hover:translate-x-1 transition-transform ml-2" />
                </div>
            </div>
         </div>
      </section>
