...
export default function CartPage() {
    ...
    const waNumber = "5563999999999"; // Fallback, ideal pegar do contexto se possível

    const handleCheckout = () => {
        if (items.length === 0) return;

        const itemsSummary = items.map(i => 
            `- *${i.name}* (Tamanho: ${i.size || 'N/A'}) - Qtd: ${i.quantity} @ R$ ${i.price}`
        ).join('\n');

        const message = encodeURIComponent(
            `Salve Baggy Hype! 🏁\n\nQuero fechar esse drop:\n\n${itemsSummary}\n\n*Total: R$ ${total.toFixed(2)}*\n\nComo prossigo com o pagamento?`
        );
            ...
            <main className="flex-1 w-full max-w-7xl mx-auto px-4 md:px-8 py-12 md:py-24">
                <header className="flex flex-col gap-2 mb-16">
                    <div className="flex items-center gap-3">
                        ...
                        <h2 className="text-[10px] md:text-xs font-black uppercase tracking-[0.5em] text-white/40 italic">Seu Drop / {count} Itens</h2>
                    </div>
                ...
                {items.length === 0 ? (
                    <div className="flex flex-col items-center justify-center py-32 gap-8 border border-white/5 bg-white/[0.02]">
                        ...
                        <div className="text-center flex flex-col gap-2">
                            <p className="text-xl font-bold uppercase tracking-widest text-white/40 italic">Seu drop está vazio.</p>
                            <p className="text-[10px] font-black uppercase tracking-[0.4em] text-white/20">O estilo não espera. Vamos começar?</p>
                        </div>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-16">
                        ...
                        <div className="lg:col-span-2 flex flex-col gap-8">
                            {items.map((item) => (
                                <div key={`${item.id}-${item.size}`} className="flex flex-col md:flex-row gap-8 border-b border-white/5 pb-8 group transition-all">
                                    <div className="relative w-full md:w-32 aspect-[3/4] bg-neutral-900 overflow-hidden border border-white/10">
                                        <Image src={item.image} alt={item.name} fill className="object-cover grayscale group-hover:grayscale-0 transition-grayscale duration-500" />
                                    </div>
                        ...
                        <div className="flex flex-col gap-8">
                            <div className="bg-white/[0.03] border border-white/5 p-8 md:p-12 flex flex-col gap-12 sticky top-32">
                                <h3 className="text-xl font-black uppercase italic tracking-tighter text-white">Resumo do <span className="text-primary italic">Drop</span></h3>
                                ...
                                <div className="flex flex-col gap-6">
                                    ...
                                    <div className="flex justify-between items-center text-[10px] font-black uppercase tracking-widest">
                                        ...
                                        <span className="text-green-500 font-bold tracking-[0.3em]">GRÁTIS 🏁</span>
                                    </div>
                                ...
                                <div className="flex flex-col gap-4">
                                    <button 
                                        ...
                                        className="group w-full py-6 bg-white text-black font-black uppercase text-xs md:text-sm tracking-widest hover:bg-primary transition-all flex items-center justify-center gap-4 hover:scale-[1.02] shadow-2xl active:scale-100 border border-white"
                                    >
                                        <ArrowRight className="w-5 h-5 group-hover:translate-x-1 transition-transform" />
                                        Fechar Drop no WhatsApp
                                    </button>
